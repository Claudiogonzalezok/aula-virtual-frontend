...
const cargarUsuarios = async (pagina = 1, termino = "") => {
setLoading(true);
try {
const { data } = await API.get(
`/usuarios?page=${pagina}&limit=15&search=${encodeURIComponent(termino)}`
);
...
setPaginaActual(1);
cargarUsuarios(1, busqueda.trim());
};
...
setToast({ show: true, mensaje: "Usuario eliminado correctamente", tipo: "success" });
  ...
  if (usuarios.length === 1 && paginaActual > 1) {
    ...
    cargarUsuarios(paginaActual - 1, terminoBusqueda);
  }
} catch (error) {
...
return ( <div> <div className="d-flex justify-content-between align-items-center mb-4"> <h2 className="fw-bold">Gestión de Usuarios</h2> <Link to="/dashboard/usuarios/nuevo"> <Button variant="success" className="shadow-sm">
➕ Nuevo Usuario </Button> </Link> </div>
          ...
          <Pagination>
            <Pagination.Prev
              onClick={() => {
                ...
                cargarUsuarios(nuevaPagina, terminoBusqueda);
              }}
            ...
            {[...Array(totalPaginas)].map((_, i) => (
              <Pagination.Item
                ...
                onClick={() => {
                  setPaginaActual(i + 1);
                  cargarUsuarios(i + 1, terminoBusqueda);
                }}
              >
                {i + 1}
              </Pagination.Item>
